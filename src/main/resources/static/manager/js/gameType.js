...
var url;
function newAreas(){
	 $('#dlg').dialog('open').dialog('setTitle','游戏类型维护');
	 $('#fm').form('clear');
	 url='/manager/gameType/put';
}
...
function destroyAreas(){
	...
	if(row){
		$.messager.confirm('Confirm','你确定要删除吗？',function(r){
			if(r){
				$.post('/manager/gameType/del',{"id":row.id},function(result){
					if(result=='success'){
						 $('#dg').datagrid('reload');
						 $.messager.show({
			                    title: 'Success',
