...
function formatWinLose(winloss){
	if(!winloss||winloss==undefined){
		return "";
	}
	if(parseFloat(winloss)>0){
		return languagePackage["js_saiguo_ying"];
	}
	if(parseFloat(winloss)<0){
		return languagePackage["js_saiguo_shu"];
	}
	if(parseFloat(winloss)==0){
		return languagePackage["js_saiguo_he"];
	}
}
...
var loadDate = function(){
	...
	function addItems(number, lastIndex) {
		...
		httpRequest.get(
				("/src/tgp/record.do"),
				{page:page, pageCount:10, beginDate:beginDate, endDate:endDate, lang:localStorage.lang},
				function(data){
					count = data.count;
					lastIndex += 5;
					var html = "";
