...
var loadDate = function(){
	...
	function addItems(number, lastIndex) {
		...
		httpRequest.get(
				("/src/ug/record"),
				{page:page, pageCount:10, beginDate:beginDate, endDate:endDate},
				function(data){
					count = data.pages;
					lastIndex += 5;
					var html = "";
					if(data.list && data.list.length > 0){
						for(var i = 0; i < data.list.length; i++){
							...
							"<div class='row'>" + 
								...
								"<div class='col-66'>" + data.list[i].betdateStr + "</div>" +
							"</div>" +
							"<div class='row' style='height: 100%;'>" +
								"<div class='col-33 font-right font-bold'>" + languagePackage["js_touzhuneirong"] + "</div>" +
								"<div class='col-66'>" + data.list[i].betinfo + "</div>" +
							"</div>" +
							"<div class='row'>" +
								"<div class='col-33 font-right font-bold'>" + languagePackage["js_peilv"] + "</div>" +
								"<div class='col-66'>" + data.list[i].betodds + "</div>" +
							"</div>" +
							"<div class='row' style='height: 100%;'>" +
								"<div class='col-33 font-right font-bold'>" + languagePackage["js_jieguo"] + "</div>" +
								"<div class='col-66'>" + formatResult(data.list[i].result) + "</label></div>" +
							"</div>" +
							"<div class='row'>" +
								"<div class='col-33 font-right font-bold'>" + languagePackage["js_touzhujine"] + "(¥)</div>" +
								"<div class='col-66'>" + formatMoney(formatMoneyIsNull(data.list[i].betamount)/100) + "</div>" +
							"</div>" +
							"<div class='row'>" +
								"<div class='col-33 font-right font-bold'>" + languagePackage["js_baoxianjine"] + "(¥)</div>" +
								"<div class='col-66'>" + formatMoney(formatMoneyIsNull(data.list[i].bxAmount)/100) + "</div>" +
							"</div>" +
							"<div class='row'>" +
								"<div class='col-33 font-right font-bold'>" + languagePackage["js_shuying"] + "(¥)</div>" +
								"<div class='col-66'>" + formatMoney((data.list[i].win)/100) + "</div>" +
							"</div>" +
							"<div class='row'>" +
							"<div class='col-33 font-right font-bold'>" + languagePackage["js_zhuangtai"] + "</div>" +
							"<div class='col-66'>" +  formatStatus(data.list[i].status) + "</div>" +
						"</div>" +
						"</div>";
						}
